...
class LexicalAnalyzer:
    ...
    def analyze(self, input_str):
        ...
        if re.match(binary_pattern, input_str):
            tokens.append(('Binary', int(input_str, 2)))
        elif re.match(octal_pattern, input_str):
            tokens.append(('Octal', int(input_str, 8)))
        elif re.match(decimal_pattern, input_str):
            tokens.append(('Decimal', int(input_str, 10)))
        elif re.match(hex_pattern, input_str):
            tokens.append(('Hexadecimal', int(input_str, 16)))
        else:
            tokens.append(('Invalid', None))

        return tokens

class SyntaxAnalyzer:
    def __init__(self):
        self.valid_bases = [2, 8, 10, 16]

    def analyze(self, tokens, input_base):
        try:
            input_base = int(input_base)
            if input_base not in self.valid_bases:
                raise ValueError(f"Invalid input base: {input_base}")

            if input_base == 2:
                valid_characters = '01'
            elif input_base == 8:
                valid_characters = '01234567'
            elif input_base == 10:
            ...
            for token_type, token_value in tokens:
                if not all(c in valid_characters for c in str(token_value)):
...
class SemanticAnalyzer:
    ...
    def analyze(self, tokens, input_base, output_base):
        try:
            input_base, output_base = int(input_base), int(output_base)

            if input_base not in self.valid_bases or output_base not in self.valid_bases:
                raise ValueError("Invalid input or output base")

            if input_base == output_base:
                raise ValueError("Input and output bases are the same")

            return True, (input_base, output_base, tokens)
        except ValueError as e:
            return False, str(e)
...
class IntermediateCodeGenerator:
    ...
    def generate_code(self, input_base, output_base, tokens):
        try:
            input_base, output_base = int(input_base), int(output_base)

            if input_base == output_base:
                self.operations.append(f"The input is already in {output_base}-based format")
            else:

                for _, token_value in tokens:
                    if input_base == 2 and output_base == 10:
                        self.operations.append(f"{int(str(token_value), 2)}")
                    elif input_base == 10 and output_base == 2:
                        self.operations.append(f"{bin(int(str(token_value)))[2:]}")
...
def convert_button_click():
    ...
    try:
        ...
        output_text.insert(tk.END, "Tokens are generated successfully by lexical.\n")
        
        
        if inpt_bas == 2 and all(c in '01' for c in input_val):
            output_text.insert(tk.END, "Syntax analyzer successfully executed.\n")
        ...
        output_text.insert(tk.END, "Intermediate code generation successful.\n")
...
window = tk.Tk()
window.title("CC Project by 005, 042, 045")
...
output_text = tk.Text(window, height=10, width=50, wrap=tk.WORD, fg="black")
